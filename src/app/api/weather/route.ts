...
export async function GET() {
  ...
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lats}&longitude=${lngs}&current=temperature_2m,precipitation,weather_code,wind_speed_10m,wind_direction_10m&hourly=precipitation_probability&forecast_days=1&timezone=Europe%2FRome`;
  ...
  const cities: CityWeather[] = ITALY_CITIES.map((city, i) => {
    ...
    return { name: city.name, lat: city.lat, lng: city.lng, temp: typeof cur.temperature_2m === "number" ? cur.temperature_2m : null, wind, windDir, precip: typeof cur.precipitation === "number" ? cur.precipitation : null, precipProb, code, label: weatherLabel(code), hailRisk: hailRisk(code, precipProb, wind) };
  });
